...
infile_path = "/Users/oweyer/Documents/procedure_distance_KGE/data/Embed_Dist/raw/"
output_path = "/Users/oweyer/Documents/procedure_distance_KGE/data/Embed_Dist/mod/"
# Load in the datasets
load(paste0(infile_path, 'final_embed_matrix.rda'))
load(paste0(infile_path, "joint_CONCEPT_opcs.rda"))

json_ids = names(final_embed_matrix)
unique_icd = unique(joint_CONCEPT_opcs$OPCS)
...
#   }
#   icd1 = icd_3digit[[i]]
#   snomed_ids_1 = joint_CONCEPT_opcs %>% filter(OPCS == icd1) %>% pull(SNOMED_id)
...
result_apply =
  lapply(1:length(short_matrix_lst), function(i) {
    print(paste0(i,' / ', length(short_matrix_lst)))
...
embed_sim = short_sim
save(embed_sim, file = paste0(output_path, "embed_sim_opcs.rda"))
