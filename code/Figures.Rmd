...
dist_embed$Value <- as.numeric(dist_embed$Value)
...
dist_naive$Value <- as.numeric(dist_naive$Value)
...
g_set1 = rbind(g1, g2, size = "first")
g_set1$widths = unit.pmax(g1$widths, g2$widths)
...
load(file = paste0(raw_data_path, "Embed_Dist/mod/embed_long_df_addChap.rda"))
...
group_icd = grep("A0+", colnames(embed_sim), value = TRUE)
group_F3_embed = embed_long_df %>% filter(OPCS1 %in% group_icd & OPCS2 %in% group_icd)
# Distribution
summary(group_F3_embed$embed_dist)
sd(group_F3_embed$embed_dist)
to_plot_mean = group_F3_embed %>% select(OPCS1, OPCS2, embed_dist) %>% 
  spread(OPCS2, value = embed_dist) %>% column_to_rownames("OPCS1") %>% as.matrix()
to_plot_mean[1,1] = NA
to_plot_mean[2,2] = NA
to_plot_mean[3,3] = NA
to_plot_mean[4,4] = NA
...
corrplot(to_plot_mean, is.corr = FALSE, method = 'color', col.lim = c(min(to_plot_mean, na.rm=TRUE), 1), type = "lower", 
...
group_icd = grep("A+", colnames(embed_sim), value = TRUE)
group_F_embed = embed_long_df %>% filter(OPCS1 %in% group_icd & OPCS2 %in% group_icd)
# Distribution
summary(group_F_embed$embed_dist)
sd(group_F_embed$embed_dist)
# Make heatmap
group_F_summary = group_F_embed %>% 
  mutate(two_digit_1 = substr(OPCS1, 1, 2),
         two_digit_2 = substr(OPCS2, 1, 2)) %>% 
...
# Output to table
write.table(chapter_summary, file = paste0(raw_data_path, "chapter_summary.txt"), quote = F, col.names = T, sep = "\t", row.names = F)
...
group_C1_embed = embed_long_df %>% filter(OPCS1 %in% group_icd & OPCS2 %in% group_icd)
# Distribution
summary(group_C1_embed$embed_dist)
sd(group_C1_embed$embed_dist)
to_plot_mean = group_C1_embed %>% select(OPCS1, OPCS2, embed_dist) %>% 
  spread(OPCS2, value = embed_dist) %>% column_to_rownames("OPCS1") %>% as.matrix()
...
# Naive
group_C1_embed = dist_final %>% filter(OPCS1 %in% group_icd & OPCS2 %in% group_icd) %>% select(OPCS1, OPCS2, naive_dist) 
group_C1_embed_add = group_C1_embed %>% dplyr::rename(OPCS2 = OPCS1, OPCS1 = OPCS2)
all_codes = unique(union(unique(group_C1_embed$OPCS1), unique(group_C1_embed$OPCS2)))
group_C1_embed_add2 = cbind(all_codes, all_codes, rep(0, length(all_codes)))
colnames(group_C1_embed_add2) = c("OPCS1", "OPCS2", "naive_dist")
...
to_plot_mean = group_C1_embed_final %>% mutate(naive_dist = as.numeric(naive_dist)) %>% 
  spread(OPCS2, value = naive_dist) %>% column_to_rownames("OPCS1") %>% as.matrix()
...
gen_embed = compare_distance_df %>% 
  select(OPCS1, OPCS2, chap_clinic1, chap_clinic2, embed_dist, gen_dist, Study_type) %>% 
  filter(OPCS1 != OPCS2) %>% filter(!is.na(embed_dist) & !is.na(gen_dist))
...
comorb_embed = compare_distance_df %>% 
  select(OPCS1, OPCS2, chap_clinic1, chap_clinic2, embed_dist, jaccard_dist) %>% 
  filter(OPCS1 != OPCS2) %>% filter(!is.na(embed_dist) & !is.na(jaccard_dist))
...
gen_tree = compare_distance_df %>% 
  select(OPCS1, OPCS2, chap_clinic1, chap_clinic2, naive_dist, gen_dist, Study_type) %>% 
  filter(OPCS1 != OPCS2) %>% filter(!is.na(naive_dist) & !is.na(gen_dist))
# N = 1806 pairs
...
comorb_tree = compare_distance_df %>% 
  select(OPCS1, OPCS2, chap_clinic1, chap_clinic2, naive_dist, jaccard_dist) %>% 
  filter(OPCS1 != OPCS2) %>% filter(!is.na(naive_dist) & !is.na(jaccard_dist))
